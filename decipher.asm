data segment
	...
	prompt db "Enter a text to decipher (32 character max) : $"
	newline db 10, 13
...
code segment
	...
	start:
		...
		; NOTE: something strange seems to happen. when i try to print the 'msg' string it doubles the printing process
		; and when i removed the code to print msg it still displays but now it displays correctly !?!?!?
		; I have no idea why this happens but hey if works it works :)

		; NOTE 2: Nevermind i figured it out. I forgot to put the null terminator "$" at the end of the 'newline' string.
		; Guess i will just leave it like that because it actually is pretty helpful.

		mov	al, [txtBuffer + 1] ; load the second byte (the length of the input) to al
		xor	ah, ah ; clear the high byte of ax to prevent any wrong readings
		...
		decipher:
			...
			sub	dl, 5 ; do the ceaser cipher (add a value (key) to our ascii character to produce a new character)
